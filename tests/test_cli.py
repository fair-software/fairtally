...
def test_single_url_no_options(invoke_cli, cli_runner: CliRunner):
    with cli_runner.isolated_filesystem():
        ...
        assert "Completed fairtally checks on 1 URLs results written to tally.html" in result.stderr
...
def test_input_from_file_to_stdout(invoke_cli, tmp_path):
    ...
    assert "fairtally progress" in result.stderr
    assert "Completed fairtally checks on 1 URLs results written to -" in result.stderr
    assert result.exit_code == 0
...
def test_json_format(invoke_cli, cli_runner):
    with cli_runner.isolated_filesystem():
        ...
        assert "fairtally progress" in result.stderr
        assert "Completed fairtally checks on 1 URLs results written to tally.json" in result.stderr
        assert result.exit_code == 0
